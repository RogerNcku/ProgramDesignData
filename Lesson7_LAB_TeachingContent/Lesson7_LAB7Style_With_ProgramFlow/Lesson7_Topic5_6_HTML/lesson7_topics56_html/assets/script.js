...
const tables={};
function initTable(id,notes){tables[id]={i:0,notes,timer:null};renderTable(id)}
function renderTable(id){const t=tables[id]; if(!t)return; const root=document.getElementById(id); if(!root)return; const rows=root.querySelectorAll('tbody tr'); rows.forEach((r,idx)=>{r.classList.toggle('show',idx<=t.i);r.classList.toggle('active-row',idx===t.i)}); const note=root.querySelector('.table-note'); if(note)note.textContent=`第 ${t.i+1} 步：${t.notes[t.i]}`; const bar=root.querySelector('.bar'); if(bar)bar.style.width=((t.i+1)/t.notes.length*100)+'%'}
function nextTable(id){const t=tables[id]; if(!t)return; t.i=Math.min(t.i+1,t.notes.length-1); renderTable(id)}
function prevTable(id){const t=tables[id]; if(!t)return; t.i=Math.max(t.i-1,0); renderTable(id)}
function resetTable(id){const t=tables[id]; if(!t)return; clearInterval(t.timer); t.i=0; renderTable(id)}
function playTable(id){const t=tables[id]; if(!t)return; clearInterval(t.timer); t.timer=setInterval(()=>{ if(t.i>=t.notes.length-1){clearInterval(t.timer);return;} t.i++; renderTable(id)},1100)}
